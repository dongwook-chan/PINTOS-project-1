...
syscall_handler (struct intr_frame *f UNUSED) 
{
  ...
  printf("syscall : reached handler\n");
  ...
  // 3 arguments
  case 3:
    printf("reached 3 arguments case\n");
...
int write (int fd, const void *buffer, unsigned size){
  printf("syscall : reached write\n");
  printf("arguments : %d, %s, %u\n", fd, buffer, size);
  switch(fd){
  case STDIN_FILENO:
    return 0;
  case STDOUT_FILENO:
    printf("syscall : reached STDOUT\n");
    putbuf(buffer, size);
    return size;	//@
  default:
    return 0;
