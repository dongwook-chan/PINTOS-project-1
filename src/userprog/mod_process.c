...
#define PUSH(buff_ptr, item, size)	{int tmp_size = size;				\
									memcpy(buff_ptr, item, tmp_size);	\
									buff_ptr += tmp_size;}

#define MEM_PUSH(item, size)	{int tmp_size = size;					\
								*esp = (uint8_t *)*esp - tmp_size;			\
								memcpy(*esp, item, tmp_size);}

#define MEM_SET(value,size) *esp = (uint8_t *)*esp - size; memset(*esp, value, size);
...
/***********************************************************************/
  ...
  char *token, *save_ptr;
  int argc;
  int esp_offset = strlen(file_name);
...
/***********************************************************************/
  ...
  for (token = strtok_r(file_name, "\r\v\n\0 ", &save_ptr), argc = 0; token != NULL; token = strtok_r(NULL, "\r\v\n\0 ", &save_ptr), ++argc) {
	  // push argv[i]
	  MEM_PUSH(token, strlen(token) + 1);
