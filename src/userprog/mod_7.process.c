...
#define PUSH(buff_ptr, item, size)	tmp_size = size;		\
					memcpy(buff_ptr++, item, tmp_size);	\

#define MEM_PUSH_UP(item, size)	tmp_size = size; printf("var dec %d\n", tmp_size);	\
				memcpy(tmp_esp, item, tmp_size); printf("memcpy %d\n", tmp_size);	\
				tmp_esp += tmp_size; printf("esp mod\n");


#define MEM_PUSH_DOWN(item, size)	tmp_size = size; printf("var dec %d\n", tmp_size);	\
					*esp -= tmp_size; printf("esp mod\n");		\
					memcpy(*esp, item, tmp_size); printf("memcpy %d\n", tmp_size);


#define MEM_SET(value,size)	tmp_size = size;		\
				*esp -= tmp_size;		\
				memset(*esp, value, tmp_size);
  ...
  void strtok_m (char *s, const char *delimiters, char *argv[MAX_ARG], int *argc, size_t *arg_len){
	...
	*arg_len = 0;
	...
	while (s_ptr != s)
	{
		...
		// 공백 제거
		while (strchr(delimiters, *s_ptr) != NULL)
			--s_ptr;
		...
		// 단어 skip
		while (strchr(delimiters, *s_ptr) == NULL && s_ptr != s){
			putchar(*s_ptr);
			--s_ptr;
		}
...
//argv
   printf("esp : %x\n", *esp);
   esp_offset_argv = arg_len + argc;
   printf("%d\n", esp_offset_argv);
   tmp_esp = *esp = *esp - esp_offset_argv;
   printf("esp : %x\n", *esp);
   printf("esp modified\n");
  //hex_dump(*esp,*esp,128 ,true);
   ...
   printf("esp : %x\n", *esp);
           ...
           PUSH(addr_ptr, &tmp_esp, sizeof(char *));
           printf("esp pushed : %x\n", tmp_esp);


	   // push argv[i]
           MEM_PUSH_UP(argv[i], strlen(argv[i]) + 1);
  ...
  printf("content of addr\n");
  for(i=0;i<argc;++i)
	printf("%x\n", addr_list[i]);


  printf("arg push competed\n");
  hex_dump(*esp,*esp,128 ,true);
...
//align
   ...
   // word align
   excess = esp_offset_argv % 4;
   if (excess){           // file_name 길이가 4바이트로 나눠 떨어지지 않으면
           MEM_SET(0, 4 - excess);
  }

  printf("%d excess, %d padded\n", excess, 4-excess);
  printf("word align competed\n");
  hex_dump(*esp,*esp,128 ,true);
  ...
  printf("argv[-1] competed\n");
  hex_dump(*esp,*esp,128 ,true);


//argv addr 
   esp_offset_addr = argc * sizeof(char *);
   printf("addr list contesnt : %x\n", addr_list);
   printf("addr list len : %d, esp_offset_addr : %d\n", strlen(addr_list), esp_offset_addr);
   MEM_PUSH_DOWN(addr_list, esp_offset_addr);


  printf("argv addr competed\n");
  hex_dump(*esp,*esp,128 ,true);

// push argv
   tmp_esp = *esp;
   MEM_PUSH_DOWN(&tmp_esp, sizeof(char **));

  printf("argv  competed\n");
  hex_dump(*esp,*esp,128 ,true);

// push arc
   MEM_PUSH_DOWN(&argc, sizeof(int));
  printf("argc  competed\n");
  hex_dump(*esp,*esp,128 ,true);
