...
process_execute (const char *file_name) 
{
  ...
  tid = thread_create (strtok_r(file_name, DELIMITERS, &save_ptr),  PRI_DEFAULT, start_process, fn_copy);
  if (tid == TID_ERROR)
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  //if(parent != child->parent || child->wait == true)
  //  return -1;


  //printf("name of child thread : %s, parent %s\n", t->name, thread_current()->name); 
  //printf("                         wating for %s\n", child->name);
  ...
  while(1){
   ...
   printf("", dying_thread.tid);
    //if(tid_to_thread(child_tid)==NULL){
    if(dying_thread.tid == child_tid){
      return -1;
    }
  ...
  // terminated by the kernel(killed due to an exception)
  // 1. 
  return -1;
}
...
#define MAX_ARG 5				// MAXimum number of ARGuments
#define MAX_ARG_ADDR MAX_ARG * sizeof(char *)	// MAXimum length of ADDResses of ARGuments
#define MAX_FILENAME 10				// MAXimum length of FILENAME



// tokenize S and save tokens to ARGV
//	count ARGC, number of arguments
//	evaluate ARG_LEN, sum of length of all arguments
...
printf("black skipped\n");
		...
		*(s++) = '\0';
        ...
        printf("tokenized argc is %d", (*argc));
	return;
  }
  ...
  void file_name_to_argv (char *s, char *argv[MAX_ARG], int *argc, size_t *arg_len){
    ...
    *arg_len = 0;
	...
	while (argv[(*argc) - 1] != NULL){
		argv[(*argc)++] =  strtok_r(s, DELIMITERS, &save_ptr);
		(*arg_len) += strlen(argv[(*argc) - 1]);
    }
  }
*/
...
/*mod_code1**********************************************************************/
  ...
  printf("load length of filename %d", strlen(file_name));
  ((char *)file_name)[strlen(file_name)] = '\0';
  file_name_to_argv(tmp_file_name,  argv, &argc, &arg_len);
...
/*mod_code2*********************************************************************/
printf("load current thread %s\n", thread_current()->name);  
    printf("arc : %d, argv : %x\n", argc, argv[0]);
/*
  if(strcmp(argv[0], "exec-once") == 0){
    printf("laod for exec-once not working exinting load\n");
    return false;
  }
*/

/*
  if(strcmp(argv[0], "child-simple") == 0){
    printf("laod for child-simple not working exinting load\n");
    printf("arc : %d, argv : %x\n", argc, argv[0]);
    return false;
  }
*/
