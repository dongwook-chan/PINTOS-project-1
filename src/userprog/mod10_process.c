...
#define MEM_PUSH(item, size)	tmp_size = size;	\
				*esp -= tmp_size; 	\
				memcpy(*esp, item, tmp_size);
...
//	S will not be modified
  void strtok_m (char *s, const char *delimiters, char *argv[MAX_ARG], int *argc, size_t *arg_len){
	*argc = 0;	// set ARGument Counter to 0 before counting
	*arg_len = 0;	// set ARGument LENgth to 0 before summation
