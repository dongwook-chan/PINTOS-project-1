...
#define MAX_FILENAME 10				// MAXimum length of FILENAME
  ...
  char *file_name_to_thread_name (char *s){
	int name_len = 0;
        char *s_ptr;
	...
	char name[name_len + 1];
        ...
        for(--name_len; name_len>=0; --name_len)
		name[name_len] = s_ptr[name_len];

	printf("thread name created : %s\n", name);
        return name;
  }
...
//	S will not be modified
  void file_name_to_argv (char *s, const char *delimiters, char *argv[MAX_ARG], int *argc, size_t *arg_len){
	int temp_argc = 0;	// set ARGument Counter to 0 before counting
	int temp_arg_len = 0;	// set ARGument LENgth to 0 before summation
	...
	while (*s != '\0')
	{
		...
		*(s++) = '\0';
		...
		// arglen 갱신
		temp_arg_len += strlen(argv[temp_argc - 1]);
...
/*mod_var**********************************************************************/
  ...
  size_t arg_len;	// sum of arguments length including NULL ptr
...
/*mod_code1**********************************************************************/
  file_name_to_argv((char *)file_name, " \r\v\n", argv, &argc, &arg_len);
